...
const EditProperty = () => {
  ...
  const removeImage = (index) => {
    const isExisting = index < (formData.images?.length || 1);
    if (isExisting) {
      const updatedImages = formData.images?.filter((_, i) => i !== index) || [];
      setFormData({ ...formData, images: updatedImages, image: updatedImages[0] || '' });
      setPreviews(prev => prev.filter((_, i) => i !== index));
    } else {
      const newIndex = index - (formData.images?.length || 1);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await updateDoc(propRef, {
        ...
        image: imageUrls[0] || '',
        images: imageUrls,
        company_name: formData.published_by_admin ? 'Relocate' : (formData.company_name || ''),
    ...
    <AdminLayout>
      <div className="max-w-4xl mx-auto">
        <div className="mb-10 flex items-center justify-between">
          ...
          <h2 className="text-3xl font-black text-gray-800">Edit Listing</h2>
        </div>

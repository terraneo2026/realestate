  ...
  <div className="bg-white rounded-[2rem] p-6 shadow-sm border border-gray-100 flex flex-col justify-between h-full group hover:shadow-md transition-all">
    <div className="flex justify-between items-start mb-4">
      <div className="flex flex-col">
        <span className="text-xs font-bold text-gray-400 tracking-tight mb-1">{title}</span>
  ...
  <div className="bg-white rounded-[2.5rem] p-8 shadow-sm border border-gray-100 flex flex-col h-full">
    ...
    <div className="flex-1 relative mt-4">
      ...
      <svg className="absolute inset-0 w-full h-full pt-2 pb-1" viewBox="0 0 700 200" preserveAspectRatio="none">
        ...
        <text x="400" y="96" textAnchor="middle" fill="white" fontSize="10" fontWeight="bold">12%</text>
      </svg>
...
export default function AdminDashboard() {
  ...
  const [leaseAlerts, setLeaseAlerts] = useState([]);
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const newStats = {
        ...
        approvedProperties: props.filter((p) => p.status === 'active' || p.status === 'approved').length,
      ...
      <div className="max-w-[1600px] mx-auto p-6 md:p-10 space-y-10">
        ...
        <div className="space-y-8">
          <div className="flex justify-between items-center">
            ...
            <Link href={`/admin/properties`} className="text-sm font-bold text-[#087c7c] tracking-tight hover:underline">
              View all
            </Link>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">

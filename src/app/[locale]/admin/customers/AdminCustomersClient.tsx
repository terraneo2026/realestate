...
import { collection, query, getDocs, where, doc, updateDoc, deleteDoc, orderBy, limit, onSnapshot, addDoc } from 'firebase/firestore';
import { 
  Users, 
  Search, 
  Filter, 
...
export default function AdminCustomersClient() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snap) => {
      ...
      fetched.sort((a: any, b: any) => {
        ...
        return getDate(b.createdAt || b.created_at).getTime() - getDate(a.createdAt || a.created_at).getTime();
      });
  ...
  const stats = useMemo(() => {
    return {
      total: users.length,
      tenants: users.filter(u => u.role === 'tenant' || u.role === 'customer').length,
  ...
  // Optimized user insights calculation
  useEffect(() => {
  ...
  const handleCreateUser = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      // For a real production app, we would use Firebase Admin SDK via an API route
      // to create a user in Firebase Auth. For now, we'll just add to Firestore
      // as a mock or if the user will sign up later with the same email.
  ...
  const filteredUsers = useMemo(() => {
    ...
    if (activeTab !== 'all') {
      filtered = filtered.filter(u => u.role === activeTab || (activeTab === 'tenant' && u.role === 'customer'));
    }
  ...
  return (
    <AdminLayout>
      ...
      <div className="bg-white rounded-[3rem] shadow-2xl shadow-gray-200/50 border border-gray-100 overflow-hidden mb-12">
                      ...
                      <td className="px-6 py-8">
                         <div className="space-y-1.5">
                            ...
                            <div className="flex items-center gap-2">
                               ...
                               <span className="text-xs font-bold text-gray-700">{user.mobile}</span>
                            </div>
                         </div>
                      </td>
                      <td className="px-6 py-8">
        ...
        <div className="p-8 border-t border-gray-50 flex justify-between items-center bg-gray-50/30">
           ...
           <div className="flex gap-2">
              <button className="p-2 bg-white border border-gray-100 text-gray-400 rounded-xl shadow-sm hover:bg-primary hover:text-white transition-all"><ChevronLeft size={16} /></button>
              <button className="p-2 bg-white border border-gray-100 text-gray-400 rounded-xl shadow-sm hover:bg-primary hover:text-white transition-all"><ChevronRight size={16} /></button>
           </div>
        </div>
      </div>
                   ...
                   <form onSubmit={handleUpdateUser} className="space-y-10">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                         ...
                         <div className="space-y-2 md:col-span-2">
                            ...
                            <textarea 
                              rows={3}
                              className="w-full px-6 py-4 bg-gray-50 border border-gray-100 rounded-2xl focus:border-primary outline-none font-bold text-sm resize-none"
                      ...
                      <div className="lg:col-span-2 space-y-10">
                         <div className="flex items-center gap-8">
                            ...
                            <div className="space-y-2">
                               ...
                               <p className="text-gray-400 font-bold flex items-center gap-2">
                                  <Phone size={16} /> {selectedUser.mobile}
                               </p>
                            </div>
                         </div>
                         ...
                         <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-8 border-t border-gray-50">
                            <div className="space-y-6">
                               ...
                               <div className="space-y-4">
                                  <div className="flex items-center gap-3">
                                     <div className="w-10 h-10 bg-gray-50 rounded-xl flex items-center justify-center text-primary"><Activity size={18} /></div>
                                     <div>
                                  ...
                                  <div className="flex items-center gap-3">
                                     <div className="w-10 h-10 bg-gray-50 rounded-xl flex items-center justify-center text-primary"><MapPin size={18} /></div>
                                     <div>
                         ...
                         <div className="pt-8 border-t border-gray-50">
                            ...
                            <div className="relative pl-8 space-y-8 before:absolute before:left-[11px] before:top-2 before:bottom-2 before:w-0.5 before:bg-gray-100">
                               <div className="relative">
                                  ...
                                  <p className="text-xs text-gray-400 mt-1">{selectedUser.createdAt?.toDate ? new Date(selectedUser.createdAt.toDate()).toLocaleDateString() : 'Recent'}</p>
                               </div>
                               <div className="relative">
                                  ...
                                  <p className="text-xs text-gray-400 mt-1">{selectedUser.updatedAt?.toDate ? new Date(selectedUser.updatedAt.toDate()).toLocaleDateString() : 'Never'}</p>
                               </div>
                            </div>
                         </div>
                      </div>
                      ...
                      <div className="space-y-8">
                         <div className="bg-gray-50 rounded-[2.5rem] p-8 border border-gray-100">
                            <h3 className="text-xs font-bold text-gray-900 tracking-tight mb-6 flex items-center gap-2">
                               <ShieldCheck size={16} className="text-primary" /> Moderation center
                            </h3>
                            <div className="space-y-4">
      ...
      {isCreating && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-black/60 backdrop-blur-md">
           <div className="bg-white w-full max-w-3xl rounded-[3rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300 flex flex-col">
              <div className="p-10 border-b border-gray-50 flex justify-between items-center shrink-0">
                 <div>
                    <h2 className="text-3xl font-bold text-gray-900 tracking-tight">Create new user</h2>
                    <p className="text-xs font-bold text-gray-400 tracking-tight mt-1">Add a new member to the Relocate platform</p>
                 </div>
                 <button onClick={() => setIsCreating(false)} className="p-3 hover:bg-gray-50 rounded-2xl transition-all text-gray-400"><X size={32} /></button>
              </div>
                 ...
                 <form onSubmit={handleCreateUser} className="space-y-8">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                       ...
                       <div className="space-y-2">
                          <label className="text-xs font-bold text-gray-400 ml-2">User role</label>
                          <select 
                            className="w-full px-6 py-4 bg-gray-50 border border-gray-100 rounded-2xl focus:border-primary outline-none font-bold text-sm appearance-none cursor-pointer"
                    ...
                    <button 
                      type="submit"
                      disabled={saving}
                      className="w-full py-5 bg-primary text-white rounded-[1.5rem] font-bold text-sm tracking-widest shadow-xl shadow-primary/20 hover:scale-[1.02] active:scale-[0.98] transition-all disabled:opacity-50 flex items-center justify-center gap-3"
                    >

...
import { collection, query, where, onSnapshot, updateDoc, doc, deleteDoc, orderBy, limit, serverTimestamp } from 'firebase/firestore';
import { 
  Bell, 
  CheckCircle2, 
  XCircle, 
  ...
  return (
    <AdminLayout>
      <div className="mb-12 flex flex-col md:flex-row md:items-end justify-between gap-8">
        <div>
          <h1 className="text-5xl font-bold text-gray-900 tracking-tight">Notifications hub</h1>
          <p className="text-gray-400 mt-2 font-semibold tracking-tight text-xs flex items-center gap-2">
             <Bell size={14} className="text-primary" /> Centralized role-based alert & notification engine
          </p>
        </div>
        <button 
            ...
            <div className="bg-white rounded-[2.5rem] p-8 shadow-2xl shadow-gray-200/50 border border-gray-100">
               ...
               <div className="space-y-2">
                  {[
                    ...
                    { id: 'system', label: 'System Alerts', icon: ShieldCheck },
                  ].map((type) => (
                    <button
                      key={type.id}
                      onClick={() => setFilterType(type.id)}
                      className={cn(
                        "w-full flex items-center gap-4 px-6 py-4 rounded-2xl text-xs font-bold uppercase tracking-tight transition-all",
            ...
            <div className="bg-gray-900 rounded-[2.5rem] p-8 text-white shadow-2xl shadow-gray-400/20">
               ...
               <div className="space-y-4">
                  <div className="flex justify-between items-center text-xs font-bold tracking-tight text-gray-400">
                     ...
                     <span className="text-white bg-red-500 px-2 py-0.5 rounded-lg text-xs">{notifications.filter(n => n.status === 'unread').length}</span>
                  </div>
                  <div className="flex justify-between items-center text-xs font-bold tracking-tight text-gray-400">
         ...
         <div className="lg:col-span-3 space-y-4">
            {filteredNotifications.length > 0 ? filteredNotifications.map((n) => (
              ...
              <div className="bg-white rounded-[3rem] p-32 text-center border border-dashed border-gray-200">
                 ...
                 <h3 className="text-xl font-bold text-gray-900 mb-2 tracking-tight uppercase">Inbox is empty</h3>
                 <p className="text-gray-400 max-w-xs mx-auto text-xs font-bold uppercase tracking-tight">No notifications for the selected criteria</p>
              </div>
            )}
         </div>
      </div>
